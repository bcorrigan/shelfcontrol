...
fn get_doc_str(field:&str,doc:&tantivy::Document, schema:&Schema) -> Option<String> {
	doc.get_first(schema.get_field(field).unwrap()).map(|v| v.text().unwrap().to_string())
}
...
fn get_query_error(query_err:QueryParserError) -> ClientError {
	match query_err {
		SyntaxError => ClientError {name: "Syntax Error".to_string(), msg: "There was a syntax error in the search string.".to_string()},
		FieldDoesNotExist(s) => ClientError {name: "Field does not Exist".to_string(), msg: format!("You searched for a field that does not exist:{}", s)},
		ExpectedInt(_) => ClientError {name: "Expected Integer".to_string(), msg: "Search argument requires an integer.".to_string()},
		AllButQueryForbidden => ClientError {name: "All But query forbidden".to_string(), msg: "Queries that only exclude (eg. \"-king\") are forbidden.".to_string()},
		NoDefaultFieldDeclared => ClientError{name: "No field declared".to_string(), msg: "You must specify a field to query.".to_string()},
		FieldNotIndexed(s) => ClientError{name: "Field unknown".to_string(), msg: format!("The field you searched for is unknown:{}", s)},
		UnknownTokenizer(_,_) => ClientError{name: "Unknown Tokenizer".to_string(), msg: "The tokenizer for the given field is unknown".to_string()},
		FieldDoesNotHavePositionsIndexed(s) => ClientError{name: "Field does not have positions indexed".to_string(), msg: format!("Field does not have positions indexed: {}", s)},
		RangeMustNotHavePhrase => ClientError{name: "Range must not have phrase".to_string(), msg: "Range must not have phrase".to_string()}
	}
}
